...
public sealed class FeatureAvailableCheck(ILogger<FeatureAvailableCheck> logger, IDbActions dbActions) : IContextCheck<FeatureAvailableCheckAttribute>
{
    ...
    public async ValueTask<string?> ExecuteCheckAsync(FeatureAvailableCheckAttribute attribute, CommandContext context)
    {
        ...
        int stationId = 0;
        if (ctx is not null)
            stationId = Convert.ToInt32(ctx.Interaction.Data.Options.Single(static o => string.Equals(o.Name, "upload-files", StringComparison.Ordinal)).Options.Single(static o => string.Equals(o.Name, "station", StringComparison.Ordinal)).Value, CultureInfo.InvariantCulture);
