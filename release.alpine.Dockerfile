...
RUN --mount=type=cache,target=/var/cache/apk,sharing=locked,id=apk-build-release \
    apk update \
  && apk upgrade
...
RUN --mount=type=cache,target=/var/cache/apk,sharing=locked,id=apk-runner-release \
    apk update \
  && apk upgrade \
