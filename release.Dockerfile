...
RUN apt update && apt upgrade -y && apt install -y --no-install-recommends iputils-ping libgssapi-krb5-2 libzstd-dev && apt autoremove --purge -y && apt clean -y && rm -rf /var/lib/apt/lists/*
