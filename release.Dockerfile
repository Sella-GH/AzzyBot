...
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked,id=apt-resolute-release \
    --mount=type=cache,target=/var/lib/apt,sharing=locked,id=apt-lib-resolute-release \
    apt-get update \
  && apt-get upgrade -y \
...
RUN --mount=type=bind,from=build,source=/packages-microsoft-prod.deb,target=/tmp/packages-microsoft-prod.deb \
    --mount=type=cache,target=/var/cache/apt,sharing=locked,id=apt-resolute-release-runner \
    --mount=type=cache,target=/var/lib/apt,sharing=locked,id=apt-lib-resolute-release-runner \
  apt-get update \
